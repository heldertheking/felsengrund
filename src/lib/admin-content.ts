...
// Shared R2-backed content store for offers and podcast episodes, replacing the old
// Keystatic + astro:content setup. Each entry is a single R2 object at
// `<offers|podcast>/<slug>.mdoc`: YAML frontmatter (the exact format the old Keystatic-managed
// files already used) followed by the Markdoc body. Keeping that format meant the original
// src/content/**/*.mdoc files could be migrated into R2 unchanged.
//
...
// Served same-origin through src/pages/media/[...key].ts, which reads the same STORAGE
// binding — so the returned URL is just a relative path, no CDN domain needed.
